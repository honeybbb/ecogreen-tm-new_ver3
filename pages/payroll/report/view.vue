...
const filteredPayrollList = computed(() => {
  // list 변수를 따로 만들 필요 없이, payrollList.value 안에서 한 번에 필터링합니다.
  const filtered = payrollList.value.filter(p =>
      (p.status > 0) && // ★ 핵심: status가 0인 데이터(저장 전)는 아예 통과시키지 않음
...
const statsInfo = computed(() => {
  // 초기화: 객체들을 미리 빈 객체로 생성해둠
  const s = { gross: 0, ded: 0, net: 0, pay: {}, deduct: {} };

  if (!payrollList.value || payrollList.value.length === 0) return s;
  ...
  filteredPayrollList.value.forEach(p => {
    ...
    if (p.payItems) {
      payItems.value.forEach(i => {
        s.pay[i.itemCd] = (s.pay[i.itemCd] || 0) + (Number(p.payItems[i.itemCd]) || 0);
      });
    }
    ...
    if (p.deductionItems) {
      deductionItems.value.forEach(i => {
        if (p.deductionFlags?.[i.itemCd] !== false) {
          s.deduct[i.itemCd] = (s.deduct[i.itemCd] || 0) + (Number(p.deductionItems[i.itemCd]) || 0);
        }
      });
    }
  });
    ...
    <div class="filter-panel">
      <div class="filter-row">
        <div class="filter-group">
          <label class="filter-label"><i class="mdi mdi-calendar-month-outline"></i> 지급연월</label>
          <input type="month" v-model="selectedYearMonth" class="filter-select" @change="getPayrollMonth"/>
        </div>
        <div class="filter-group">
          <label class="filter-label">
            <i class="mdi mdi-account-cash"></i> 지급일자
          </label>
