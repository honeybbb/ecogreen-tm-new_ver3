...
const applyDeductionLogic = async (row, item, base, rates) => {
  ...
  let amt = 0;
  if      (item.itemCd === '04002003') amt = base * (rates.pension / 100);
  else if (item.itemCd === '04002001') amt = base * (rates.health / 100);
  else if (item.itemCd === '04002002') {
    // 장기요양: 반드시 건강보험료 기준
    const healthAmt = row.deductions['04002001'] || 0;
    amt = healthAmt * (rates.longTerm / 100);
  }
  else if (item.itemCd === '04002004') amt = base * (rates.employment / 100);
  else return; // 기타공제 등 수동 입력 항목은 건드리지 않음
        ...
        <table class="data-table">
          ...
          <tr v-for="(p, i) in pagedPayrollList" :key="p.idx" class="data-row">
            ...
            <td v-for="item in deductionItems" :key="item.itemCd" class="amount-cell">
              <input v-if="p.deductions" type="number" v-model.number="p.deductions[item.itemCd]" @input="markAsDraft(p)" class="inline-input">
            </td>

            <td v-for="item in checkedItems" :key="'ded-chk-'+item.itemCd" class="text-center deduction-check-cell">
              <label class="checkbox-wrapper">
                <input type="checkbox" v-model="p.deductionFlags[item.itemCd]" @change="markAsDraft(p); calculateInsurances(p, item)" class="custom-checkbox">
              </label>
            </td>
          </tr>
          <tr v-if="filteredPayrollList.length === 0">
            <td :colspan="7 + payItems.length + (deductionItems.length * 2)" class="empty-state">
